...
function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      {settings && <section className="glass settings"><div className="section-title"><h2>Connection settings</h2><button className="icon-button" aria-label="Close settings" onClick={() => setSettings(false)}><X size={16}/></button></div><p>Open Anki with AnkiConnect enabled on this computer. Allow <code>{location.origin}</code> in its webCorsOriginList.</p><div className="settings-fields"><label>API key (if configured)<input type="password" value={key} autoComplete="off" onChange={e => setKey(e.target.value)} placeholder="Optional · kept in memory"/></label><label>Study day starts at<select value={rollover} onChange={e => setRollover(Number(e.target.value))}>{Array.from({ length: 24 }, (_, h) => <option key={h} value={h}>{String(h).padStart(2, '0')}:00</option>)}</select></label><button className="refresh-button" onClick={() => void refresh()} disabled={loading}>Reconnect</button></div><p className="small">Match the next-day start time in Anki’s preferences. Times use this computer’s timezone.</p></section>}
      ...
      <footer className="footer"><span aria-live="polite">{loading ? <><RefreshCw size={12} className="spinning"/>{message}</> : data ? <><Check size={12}/>Updated {new Date(data.loadedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}{error ? ' · stale' : ''}</> : 'Waiting for connection'}</span><span>Made for a little more focus.</span></footer>
